server_name="server {
    listen 80;
    location / {
        proxy_pass http://54.158.20.236:8000;
...
destdir=/etc/nginx/sites-available/fdfdf
...
sudo apt-get update
cd /var/www
sudo git clone fdfsf

cd /etc/nginx/sites-available/
sudo rm default
sudo touch fdfdf
sudo sh -c "echo '$server_name' >> '$destdir'"
sudo ln -s /etc/nginx/sites-available/fdfdf /etc/nginx/sites-enabled/fdfdf
cd ../sites-enabled/
sudo rm default
